...
const CreateProfileOne: React.FC<CreateProfileOneProps> = ({
  ...
  useEffect(() => {
    const kycData = watch("kyc");
    if (kycData) {
      const score =
        kycData.filter((item) => item.document !== null).length || 0;
      setKycScore(score * 20); // Assuming each document gives 20 points
    }
  }, [watch("kyc")]);
      ...
      <Form {...form}>
        ...
        >
          ...
          {activeStep === 1 && (
           ...
           {defaultValues.kyc.map((kyc, index) => (
               ...
               <div className="flex items-center flex-grow">
                 ...
                 <div className="px-4">
                   <Button
                     ...
                     disabled={loading || (kycStatus && kycStatus.some((status) => status.document_type === kyc.type && status.document_type))}
                   >
                     {kycStatus && kycStatus.some((status) => status.document_type === kyc.type && status.document_type)
                       ? "Uploaded"
                       : `Upload ${kyc.type}`}
                   </Button>
                 </div>
